...
- Push All is additive-only in current code: the Studio plugin now exports the allowed services service-by-service in `PUSH_BATCH` uploads of 50 instances, logs per-service progress, and the daemon writes those batches to disk through additive sync hooks without delete messages, delete paths, or directory-removal flows during Push All.
